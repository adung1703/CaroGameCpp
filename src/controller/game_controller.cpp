...
void GameController::runGame() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 2: { // Play with BOT
                ...
                do {
                    ...
                    if (botChoice >= 1 && botChoice <= 3) {
                        ...
                        if (botChoice == 1) botDifficulty = Difficulty::EASY;
                        else if (botChoice == 2) botDifficulty = Difficulty::NORMAL;
                        else botDifficulty = Difficulty::HARD;
            ...
            case 5: { // Guild (Display All Players)
...
void GameController::handlePlayerTurn() {
    consoleView.displayMessage("Player " + string(1, currentPlayer->getSymbol()) + "'s turn (" + currentPlayer->getName() + "):"); // Use View
    pair<int, int> move = currentPlayer->getMove();
    int row = move.first;
    int col = move.second;

    if (gameBoard->placeMove(row, col, currentPlayer->getSymbol())) { // Use pointer to access Singleton
        addToMoveHistory(row, col);
        if (checkGameOver(row, col)) return;
    } else {
...
bool GameController::checkGameOver(int lastMoveRow, int lastMoveCol) {
    ...
    if (row != -1 && col != -1 && gameBoard->checkWin(row, col, currentSymbol)) { // Use pointer to access Singleton
        gameRunning = false;
        ...
        return true;
    } else if (gameBoard->isBoardFull()) { // Use pointer to access Singleton
        gameRunning = false;
...
void GameController::showPlayerInformation() {
    ...
    if (player) {
        consoleView.displayPlayerInfo(*player); // Use View to display player info
    } else {
        consoleView.displayMessage("Player '" + playerName + "' not found."); // Use View
    }
